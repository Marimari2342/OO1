package ar.edu.unlp.info.oo1.ej22_correo;

import java.util.ArrayList;
import java.util.List;

import main.java.ar.edu.unlp.info.oo1.ej13_correo.Email;
import unlp.oo1.bag.BagImpl;
...
public class Carpeta {
    ...
    // --> PUNTO 22
    //cantidad de mails que tiene una carpeta
    public int cantidadMails(){
        return this.getEmails().size();
    }

    //cantidad de mails por categoría
    public Bag<String> categorizarEmails() {
		Bag<String> bolsa = new BagImpl<String>();
		
        this.getEmails().stream().forEach(mails -> {
			if (mails.espacioOcupado() <= 300) {
				bolsa.add("Pequeño");
			} else if (mails.espacioOcupado() > 300 && mails.espacioOcupado() <= 500) {
				bolsa.add("Mediano");
			} else {
				bolsa.add("Grande");
			}
