...
public class JobSchedulerTest {
    ...
    // Crear los JobSchedulers con las estrategias correspondientes
    private JobScheduler newFifoScheduler() {
        return new JobScheduler(new FifoStrategy());
    }

    private JobScheduler newLifoScheduler() {
        return new JobScheduler(new LifoStrategy());
    }

    private JobScheduler newPriorityScheduler() {
        return new JobScheduler(new HighestPriorityStrategy());
    }

    private JobScheduler newEffortScheduler() {
        return new JobScheduler(new MostEffortStrategy());
    }
    ...
    @Test
    void testSchedule() {
        JobScheduler aScheduler = newFifoScheduler();  // Ahora usamos una estrategia concreta
